(function (root, factory) {
  ...
  function renderPortableComparison(container, delegated) {
    ...
    var result = delegated.result;
    var freshProjection = result.fresh_projection;
    var leftState = freshProjection && freshProjection.verification && freshProjection.verification.left && freshProjection.verification.left.state;
    var rightState = freshProjection && freshProjection.verification && freshProjection.verification.right && freshProjection.verification.right.state;
    var comparisonState = freshProjection && freshProjection.comparison;
    var freshVerified = leftState === 'VERIFIED' && rightState === 'VERIFIED' && comparisonState;
    var status = freshVerified ? 'FRESHLY_VERIFIED' : (leftState === 'REJECTED' || rightState === 'REJECTED' ? 'REJECTED' : 'UNVERIFIED');
    var statusLabel = freshVerified ? 'FRESHLY VERIFIED' : status;
    var doc = container.ownerDocument;
    ...
    root.setAttribute('data-portable-comparison-status', status);
    root.setAttribute('aria-label', freshVerified ? 'Portable Trail Comparison inspection' : 'Portable Trail Comparison diagnostic');
    var header = el(doc, 'header', 'replay-inspection-header');
    var heading = el(doc, 'div', 'replay-inspection-heading');
    heading.appendChild(el(doc, 'span', 'replay-inspection-kicker', 'REPLAY / PORTABLE TRAIL COMPARISON'));
    heading.appendChild(el(doc, 'strong', 'replay-inspection-proof-state', statusLabel));
    header.appendChild(heading);
    root.appendChild(header);
